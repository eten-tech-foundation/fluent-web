...
const source = [1, 2, 3, 4, 5].map(verseNumber => ({ id: verseNumber + 100, verseNumber, text: '' }));

describe('pericope suggestion scope', () => {
  it('skips groups without source verses when choosing the next group', () => {
    expect(pericopeSuggestionScope(groups, 2, source)).toEqual({ verseNumbers: [1, 2], nextVerseNumbers: [3, 4] });
  });
  it('does not prefetch past the last renderable group', () => {
    expect(pericopeSuggestionScope(groups, 5, source)).toEqual({ verseNumbers: [5], nextVerseNumbers: [] });
  });
  it('requests nothing for an unknown active verse', () => {
    expect(pericopeSuggestionScope(groups, 9, source)).toEqual({ verseNumbers: [], nextVerseNumbers: [] });
  });
});
