/** Original ladder test names retained; policy now drives the player's request interface. */
...
import type { TtsRecoveryTiming } from './ttsRecoveryTiming';
...
// ---------------------------------------------------------------------------
// superviseTtsStrategy — HEAD re-probe ladder (§6.1)
// ---------------------------------------------------------------------------

describe('superviseTtsStrategy — element error ladder', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  afterEach(() => {
  ...
  const setup = (
    ...
    const detach = superviseTtsStrategy({
      element,
      audioUrl: CLIP_URL,
  ...
  it('retries are capped per failure class; the third failure surfaces the typed error (CB1)', async () => {
    ...
    expect(onFailure.mock.calls[0][0]).toMatchObject({
      message: 'TTS mid-stream retries exhausted',
    });
  });
...
// ---------------------------------------------------------------------------
// superviseTtsStrategy — stall watchdog (N4, §6.1 platform risk)
// ---------------------------------------------------------------------------

describe('superviseTtsStrategy — stall watchdog', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  afterEach(() => {
  ...
  it('exhausted wait-for-compressed polling surfaces the typed stall failure', async () => {
    ...
    expect(onFailure).toHaveBeenCalledTimes(1);
    expect(onFailure.mock.calls[0][0]).toMatchObject({
      message: 'TTS wait-for-compressed poll budget exhausted',
    });
  });
});
