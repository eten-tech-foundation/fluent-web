...
- **Confirm dialog on `[Ignore Everywhere]`** (revised #278): the card asks for it because the action reads as global and irreversible. We honor the request **and** keep the action reversible via `[Undo ▾]` below — the dialog guards the deliberate click; the undo guards a mistake that slips past it. The two are not redundant: confirm is pre-action friction, undo is post-action recourse. (We considered dropping undo to match a strict "cannot be undone" reading, but the cascade already supports clean reversal at the right scope, and there is no explicit directive to remove it; the final PR is reviewed again before merge.)
...
- **Why undo despite the "cannot be undone" wording.** The card's confirm-dialog copy ("cannot be undone") is product framing for the *commitment* of a global ignore, not a storage constraint. With the three-layer cascade, "undo" is well-defined at each scope (delete the global entry, or `surface`-override one occurrence), so we surface it rather than stranding the translator. This is the one place we keep more capability than the literal card; flagged in §12 for sign-off.
...
- **Clean semantic separation** — `self/*` is unambiguously "me," leaving the `users/*` domain for (future) admin-style tooling that operates on *other* users by id.
...
| `PUT` | `/self/settings` | `authenticateUser` only | `{ settings: UserSettings }` | `200` saved state; `400` on schema violation |
...
*Prepared against fluent-web `main` (post `2e936d1`), fluent-api `main` (post `7c7f63d`), and the approved fluent-api proposal (PR #173). Revised 2026-06-16 after the first review round on PR #305. Author: JEdward7777.*
