...
> [!CAUTION]
> If production is down hard and even a QA pass is too slow, run **Promote to Production** with `skip_qa_check` ticked. It deploys a tag no environment has tested. It logs a warning naming you as the actor and still requires `production` reviewer approval — a second person is looking. Treat every use as something to raise at the incident review.
